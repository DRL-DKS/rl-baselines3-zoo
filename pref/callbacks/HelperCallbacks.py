...
import numpy as np
import torch
from gym.wrappers import RecordVideo, NormalizeObservation
...
from pref.website.query_database import QueryDatabase
...
class InitializeRewardModel(BaseCallback):
    """
    Runs initial training of the reward model and add Tensorboard writer to the human critic.
    The agents training need to start before the tensorboard log file is created, hence we add a callback
    which creates a writer using the agents logging.

    :param hc: (HumanCritic) the human critic class to perform logging
    :param agent: (BaseAlgorithm) Stablebaseline agent
    :param verbose: (int) Verbosity level 0: not output 1: info 2: debug
    """
    def __init__(self, hc, agent, meta_data, init_prefs_n, reward_model_save, initial_training_epochs, truth, verbose=0):
        super(InitializeRewardModel, self).__init__(verbose)
        self.hc = hc
        self.agent = agent
        self.initial_training_epochs = initial_training_epochs
        self.meta_data = meta_data
        self.init_prefs_n = init_prefs_n
        self.reward_model_save = reward_model_save
        self.truth = truth

    def _on_training_start(self) -> None:
        self.hc.writer = SummaryWriter(self.agent.logger.get_dir())

        # TODO: Make this work with different environments
        queries = self.hc.get_queries_to_be_trained(number_of_queries=self.init_prefs_n, experiments=1, truth=self.truth)
        o1, o2, prefs = self.hc.generate_data_for_training(queries)
        tensor_o1 = torch.Tensor(o1)
        tensor_o2 = torch.Tensor(o2)
        tensor_prefs = torch.Tensor(prefs)
        my_dataset = TensorDataset(tensor_o1, tensor_o2, tensor_prefs)
        my_dataloader = DataLoader(my_dataset, batch_size=self.hc.batch_size, shuffle=True)
        self.hc.clear_segment()
        self.hc.train_dataset(my_dataloader, self.meta_data, self.initial_training_epochs)
        self.hc.save_reward_model(self.reward_model_save + "-Reward")
...
class UpdateRewardFunction(BaseCallback):
    ...
    def _on_training_start(self) -> None:
        ...
        trajectories, critical_points = self.collect_segments_with_critical_points(self.agent, 100000, traj_to_collect)
    ...
    def _on_step(self) -> bool:
        ...
        self.agent.save(self.env_name + "-loop2")
    ...
    def collect_segments(self, model, env, test_episodes=5000, n_collect_segments=0):  # evaluate
        ...
        for e in range(test_episodes):
            ...
            segment_reward = 0
            while not done:
                action, _ = model.predict(obs, deterministic=True)  # TODO test deterministic and non-deterministic
    ...
    def collect_segments_with_critical_points(self, model, test_episodes=5000, n_collect_segments=0, extra_env=None):  # evaluate
        ...
        if self.env_name == 'unity-env':
            channel = EngineConfigurationChannel()
            unity_env = UnityEnvironment('envs/snappy_rays', worker_id=1, side_channels=[channel])
...
class UpdateRewardFunction2(BaseCallback):
    """
    Callback to periodically gather query feedback and updates the reward model.
    ...
    def __init__(self,
                 ...
                 seed=12345,
                 max_queries=3000):
        super(UpdateRewardFunction2, self).__init__(verbose)
        self.n_queries = n_queries
        self.hc = hc
        self.agent = agent
        self.initial_reward_estimation_epochs = initial_reward_estimation_epochs
        ...
        self.largest_rew_threshold = largest_rew_threshold
        self.n_initial_queries = n_initial_queries
        self.hc_expert = hc_expert
        self.seed = seed
        self.max_queries = max_queries

        if self.env_name == 'unity-env':
            channel = EngineConfigurationChannel()
            unity_env = UnityEnvironment('envs/snappy_rays', side_channels=[channel], worker_id=1)
            channel.set_configuration_parameters(time_scale=30.0)
            self.env = UnityToGymWrapper(unity_env, uint8_visual=False, allow_multiple_obs=False)
    ...
    def _on_training_start(self) -> None:
        ...
        # Train reward model (How much?)
        self.hc.writer = SummaryWriter(self.agent.logger.get_dir())
        self.hc_expert.writer = SummaryWriter(self.agent.logger.get_dir() + "2")
        traj_to_collect = self.n_initial_queries * 5 * 2
        trajectories, critical_points = self.collect_segments_with_critical_points(self.agent, 100000, traj_to_collect)
        trajectories = self.hc.segments[:self.hc.segments_size]

        self.hc.generate_preference_pairs(trajectories, truth=self.truth, number_of_queries=self.n_initial_queries)
        self.hc_expert.generate_preference_pairs(trajectories, truth=100, number_of_queries=self.n_initial_queries * 2)

        self.train_reward_model(self.hc, self.initial_reward_estimation_epochs)
        self.train_reward_model(self.hc_expert, self.initial_reward_estimation_epochs)
    ...
    def _on_step(self) -> bool:
        ...
        if self.hc.pairs_size < self.max_queries:
            self.agent = self.agent.load(self.env_name + "-loop.zip")
            # test_agent(agent, 3)
            traj_to_collect = self.n_queries * 5 * 2
            trajectories, critical_points = self.collect_segments_with_critical_points(self.agent, 100000, traj_to_collect)
            trajectories = self.hc.segments[:self.hc.segments_size]

            self.hc.generate_preference_pairs(trajectories, truth=self.truth, number_of_queries=self.n_queries)
            self.hc_expert.generate_preference_pairs(trajectories, truth=100, number_of_queries=self.n_queries * 2)

            self.train_reward_model(self.hc, self.reward_training_epochs)
            self.train_reward_model(self.hc_expert, self.reward_training_epochs)
            self.hc.save_reward_model(self.env_name + "-loop")
            self.hc.save_reward_model(self.env_name + "-loop")
        return True
    ...
    def train_reward_model(self, reward_model, training_epochs):
        o1, o2, prefs = reward_model.get_all_preference_pairs()
        # TODO: Change all these places to not convert from numpy to tensor...
        tensor_o1 = torch.Tensor(o1)
        tensor_o2 = torch.Tensor(o2)
        tensor_prefs = torch.Tensor(prefs)
        ...
        my_dataset = TensorDataset(tensor_o1, tensor_o2, tensor_prefs)
        my_dataloader = DataLoader(my_dataset, batch_size=self.hc.batch_size, shuffle=True)
        reward_model.train_dataset(my_dataloader, None, training_epochs)
    ...
    def collect_segments(self, model, env, test_episodes=5000, n_collect_segments=0):  # evaluate
        ...
        for e in range(test_episodes):
            ...
            segment_reward = 0
            while not done:
                action, _ = self.model.predict(obs, deterministic=False)  # TODO test deterministic and non-deterministic
                #action = self.model.policy(obs)
                #obs_tensor = obs_as_tensor(self._last_obs, self.device)
                #actions, values, log_probs = self.model.policy(obs)
    ...
    def collect_segments_with_critical_points(self, model, test_episodes=5000, n_collect_segments=0, extra_env=None):  # evaluate
        ...
        for e in range(test_episodes):
            ...
            while not done:
                ...
                #obs = obs_as_tensor(obs, device=self.model.device)
                #self.model.policy(obs_as_tensor(obs, device=self.model.device))
...
class UpdateRewardFunctionRealHuman(BaseCallback):
    ...
    def collect_segments(self, model, n_queries=5):  # evaluate
        def start_recording(agent_timestep):
            if (agent_timestep == trigger) or (agent_timestep == trigger):
                ...
                if agent_timestep == trigger + self.traj_lenght + 1 or agent_timestep == trigger + self.traj_lenght + 1:
                    print("Finished recording: " + str(agent_timestep - 1))
                return False

        timestep = 0
        total_segments, triggers, traj_segment = [], [], []
        ...
        collecting_env = RecordVideo(collecting_env, self.video_location, step_trigger=start_recording, video_length=self.traj_lenght, name_prefix=video_prefix)
        ...
        while True:
            ...
            while not done:
                ...
                if trigger <= timestep < trigger + self.traj_lenght:
                    if timestep == trigger:
                        print("Starting to collect: " + str(timestep))
                    traj_segment.append(np.concatenate((obs.squeeze(), action)))

                if len(traj_segment) == self.traj_lenght:  # TODO see if same logic applies
                    ...
                    if len(total_segments) >= n_queries:
                        ...
                        video_names = []
                        for tr in triggers:
                            video_names.append(video_prefix + "-step-" + str(tr) + ".mp4")

                        return total_segments, video_names

                    start = trigger + self.traj_lenght
                    end = trigger + self.traj_lenght + 1000
